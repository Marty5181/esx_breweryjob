...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = 'Firemni vozidlo',  value = 'bison3'},
		}
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_brewery', 'Pivovar', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx_breweryjob:hasEnteredMarker', function(zone)
	if zone == 'ChmelFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'brewery'  then
		CurrentAction     = 'chmel_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementPivo' and PlayerData.job ~= nil and PlayerData.job.name == 'brewery'  then
		CurrentAction     = 'pivo_traitement'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementChmeljus' and PlayerData.job ~= nil and PlayerData.job.name == 'brewery'  then
		CurrentAction     = 'chmeljus_traitement'
...
AddEventHandler('esx_breweryjob:hasExitedMarker', function(zone)
	...
	if (zone == 'TraitementPivo' or zone == 'TraitementChmeljus') and PlayerData.job ~= nil and PlayerData.job.name == 'brewery' then
